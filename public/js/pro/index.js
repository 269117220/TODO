var f=function(_t){
    ...
    */    
    var _onsearchTODO=(_event)=>{
        ...
        if(!numArr['短期'])
            delete globalTempDate['短期'];
        if(!numArr['长期'])
            delete globalTempDate['长期'];
    ...
    var _onLoginShow=(_event)=>{
        ...
        if(_target.innerHTML!='登出'){
            ...
            if(_target.parentNode.parentNode.style.right=='0px'){
                ...
                _target.parentNode.parentNode.style.right='0px';
        }else{
            /*登出的情况*/
            var loginDiv=_e._$get("loginDiv");
    ...
    *获取全部记录的数目
    */
    var getNum=(data,cName)=>{
        var objTemp=data[cName],num=0;
    ...
    *获取搜索记录的数目
    */
    var getSearchNum=(data,cName)=>{
        var objTemp=data[cName],num=0;
        for(var keyName in objTemp){
            objTemp[keyName].forEach((TODO,index)=>{
                if(TODO!='')
                    num++;
            });
        }
    ...
    */
    var _onItemRemove=(_event)=>{
        ...
        if(searchFlag&&!searchNum) 
            setHtml(searchData,'search');
        else
            setHtml(globalTempDate,'changeData');
        updateTODO('删除成功');
    }
    globalTempDate=JSON.parse(JSON.stringify(data));
    /*
    ...
    */
    function changeClass(_target,value,otherValue){
        ...
        if(classStr.includes(value)){
            ...
            if(value=='fa-square-o')
                _target.nextSibling.nextSibling.style="text-decoration:line-through;color:#D9D9D9";
        }else{
            _target.setAttribute('class',classStr.replace(otherValue,value));
            if(value=='fa-square-o')
                _target.nextSibling.nextSibling.style="";
        };
        var classStr=_target.getAttribute('class');
    }
    /*
    ...
    */
    var _onChangeChoose=(_event)=>{
        ...
        if(gChooseArr[arr[0]]==arr[0]+'_all'){
            if(!searchFlag)
                globalTempDate[arr[0]]=JSON.parse(JSON.stringify(data[arr[0]]));
            else
                globalTempDate[arr[0]]=JSON.parse(JSON.stringify(searchData[arr[0]]));
        }
    ...
    */
    ...
    function setHtml(data,_target){
        ...
        _parent.innerHTML=(_html.trim()=='')?'<span style="color:gray;margin-top:100px;text-align:center">'+
        '分为短期/长期记录<br/>'+
        '模糊查询TODO记录<br/>'+
        '优先级更高的记录可以置顶<br/>'+
        '双击编辑记录<br/>'+
        ...
        if(_target=='search'){
            ...
            if(_categoryDivs.length==0)
                _parent.innerHTML='<span class="fa fa-warning" style="color:gray;margin-top:100px;text-align:center;font-size:25px;">&nbsp;无相关记录</span>';
        }
    }
    setHtml(data);
    /*
